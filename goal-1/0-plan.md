...
| §6.1 | Discrete billiard states use a unit grid, radius `1/√2`, unit velocity, integral observation times, and a restriction to right-angle collisions | 11 | Stage 11 audit: this is continuous hard-ball/specular-mirror mechanics sampled at integer times, not a complete discrete global update rule. Occupancy alone omits velocity, and simultaneous/multi-contact behavior is unspecified. The accepted formal model must expose a legal directed/event subset and must not claim continuous mechanics |
| §6.2 | `(p,q) ↦ (pq, ¬p q, p ¬q, pq)` is an equivalence onto four valid rail states and preserves ball count | 11 | Stage 11 target locked: heterogeneous equivalence onto the exact four-state range, with a constrained inverse, cardinality obstruction to the raw four-rail reading, and separate ball-count preservation |
| §6.3 | Interaction-gate AND/NOT realization plus universality with constants and valid routing/timing | 11 | Stage 11 separates the exact local rail semantics and a selected Figure 14 sampled refinement from the unproved arbitrary-layout composition claim. Mirror routes and timing-conflict predicates are modeled only at the sampled-path level |
| §6 introduction and §6.4 | Any conservative-logic circuit has a billiard-ball realization | 11 | Stage 11 audit rejects derivation of this strong claim from primitive endpoint tables. A full result still needs directed global dynamics, collision/mirror scheduling, continuous clearance, bridge crossover refinement, and a compositional layout theorem |
| §6.4, Figs. 16–18 | The switch `(c,x) ↦ (c,cx,¬c x)` is an equivalence onto four valid three-rail states, and collision layouts refine switch/Fredkin semantics | 11 | Stage 11 target locks the switch's four-state constrained equivalence and raw-cardinality obstruction. Figure 18 is an obstruction/documentation target because its caption omits steering/timing mirrors and unit wires; the bridge and claimed trivial crossovers still require checked intermediate trajectories |
...
| CL-007 | The interaction and switch gates use constrained, unequal-width rail encodings. They preserve balls/ones but not the number of zero-valued physical rails and are exceptions to the ordinary balanced-port gate type. | Stage 11 contract fixed: use explicit valid-output subtypes, heterogeneous equivalences, and heterogeneous `WeightPreserving` theorems; prove cardinality obstructions to `Bool² ≃ Bool⁴` and `Bool² ≃ Bool³`; never construct an ordinary `Conservative` instance. |
| CL-008 | Figure 18 explicitly omits steering/timing mirrors and unit wires, but identifies bridge crossovers and calls the others trivial; clearance and simultaneous collision scheduling are additional formalization obligations rather than quoted omissions. | Stage 11 contract corrected after reading Figures 12–18: prove only a sampled four-tick Figure 14 layout and explicit route/timing-conflict facts. Keep continuous clearance, nontrivial bridge refinement, Figures 17–18 completion, and arbitrary layout composition unresolved until a directed global mechanics and every omitted route are supplied. |
...
been re-audited.  They specify continuous hard-ball/specular-mirror mechanics
sampled at integral times, but not a total discrete global transition:
...
- Record a checked under-specification boundary for raw illegal collision
  states and for Figure 18's erased timing/layout data.  Do not claim the

...
- Stage 12 begins from clean synchronized commit `1047322`.  The finite root,
  ...
  guardrail probe plus Stages 2--11) build separately.  At that baseline there
  was no final public-consumer examples leaf, aggregate main-result axiom audit,
  or Stage 12 evidence report. Stage 12 now adds all three, closes the narrow
  derived-`N₀` and arbitrary-wire-length correspondence gaps, and repairs
  generic declaration ownership; final clean verification is recorded below.
...
- The reversible-only all-zero `n`-bit result-register variant cites an external
  construction that has not been reconstructed, and the fixed-CPU universality
  claim still needs a universal encoding and simulation theorem.  Finite
  iteration of a supplied transition proves neither claim.
...
| §§2.5, 7.1 | Acyclic/equal-latency combinational fragment and one-to-one composition | 4, 7 | `ConservativeLogic.Circuit` is a corrected one-to-one feed-forward grammar; `ConservativeLogic.Circuit.PathDelay`, `ConservativeLogic.Circuit.HasLatency`, and `ConservativeLogic.Circuit.MeetsPaperCombinationalTiming` formalize the equal-unit-wire-path clause. `ConservativeLogic.Circuit.pathDelay_inverse_iff` and `ConservativeLogic.Circuit.meetsPaperCombinationalTiming_inverse_iff` prove exact reversal/preservation for that grammar only. No graph correspondence, feedback semantics, or physical routing result is claimed |
| §2.5 | Reversibility and conservation are independent | 2 | Proved semantically for ordinary Boolean endomaps by `ConservativeLogic.Independence.reversible_not_weightPreserving` and `ConservativeLogic.Independence.weightPreserving_not_reversible`; this does not assert a literal circuit realization |
| §3, Fig. 5 | Realization partitions source/argument and result/sink, fixes constants independently of the argument, and permits argument-dependent garbage | 5 | `ConservativeLogic.Realization.Layout` and `ConservativeLogic.Realization.Realizes` give a stronger explicit five-block specialization: fixed source and returned-clean scratch, universal argument, selected result, explicit argument-indexed garbage, and equality of the complete boundary state |
...
| §6.1 | Integral-time lattice samples retain directed unit moves and a selected right-angle local event | 11 | Corrected discrete abstraction implemented. `ConservativeLogic.Billiard.Grid.Route` retains four directed unit moves, `ConservativeLogic.Billiard.Collision.AllowedState` restricts local masks, and independent `ConservativeLogic.Billiard.ScatteringLayer.Configuration` has `ConservativeLogic.Billiard.ScatteringLayer.Configuration.stepEquiv` and count law `ConservativeLogic.Billiard.ScatteringLayer.Configuration.totalBallCount_step`. `ConservativeLogic.Billiard.ScatteringLayer.illegalThreeBallMask_not_allowed` plus the diagnostic `ConservativeLogic.Audit.Billiard.tripleCandidates_conflict` record why this is not a global mechanics |
| §6.1 | Radius `1/√2`, continuous unit velocity, elastic impact, preservation of a horizontal velocity component, and vertical-alignment synchronization | — | Out-of-model. Lean contains one lattice move per sample and parameterized sampled-distance predicates, but no radius, real-valued velocity, continuous collision law, or Figure 18 alignment theorem. The checked threshold-`2` examples are sampled certificates, not those mechanics |
| §6.2 | `(p,q) ↦ (pq, ¬p q, p ¬q, pq)` is an equivalence onto four valid rail states and preserves ball count | 11 | Resolved exactly by `ConservativeLogic.Billiard.Interaction.equiv`, `ConservativeLogic.Billiard.Interaction.decode`, `ConservativeLogic.Billiard.Interaction.encode_weightPreserving`, `ConservativeLogic.Billiard.Interaction.card_validOutput`, and `ConservativeLogic.Billiard.Interaction.no_raw_equiv`. `ConservativeLogic.Billiard.Collision.map_embed` realizes the complete initialized four-channel slice, and `ConservativeLogic.Billiard.Figure14.output_refines_collision` supplies the selected four-tick coordinate trace |
| §6.3 | Interaction-gate AND/NOT realization plus universality with constants and valid routing/timing | 11 | `ConservativeLogic.Billiard.Interaction.encode_a` and `ConservativeLogic.Billiard.Interaction.not_with_true_source` check the named logic slices with the occupied constant explicit. `ConservativeLogic.Billiard.Grid.mirrorTurn_reflects`, `ConservativeLogic.Billiard.Grid.fourTickDetour_reflects`, `ConservativeLogic.Billiard.Grid.simultaneous_crossing_conflict`, `ConservativeLogic.Billiard.Grid.oneTickStagger_not_sampleClearance`, and `ConservativeLogic.Billiard.Grid.twoTickStagger_sampleClearance` check selected sampled routing obligations. Arbitrary mirror delay, the bridge, routing composition, and physical universality remain open |
| §6 introduction and §6.4 | Any conservative-logic circuit has a billiard-ball realization | 11 | Open, not theoremized. Diagnostic theorems `ConservativeLogic.Audit.Billiard.collision_not_unionPreserving` and `ConservativeLogic.Audit.Billiard.no_wirePerm_collision` show that collision cannot be replaced by independent union/routing, while the public equal-endpoint short/detour routes show endpoint algebra does not fix timing. A whole-circuit result still needs directed global dynamics, schedules, continuous clearance, the bridge, and a compositional layout compiler |
| §6.4, Figs. 16–18 | The switch `(c,x) ↦ (c,cx,¬c x)` is an equivalence onto four valid three-rail states, and collision layouts refine switch/Fredkin semantics | 11 | The logical switch is resolved by `ConservativeLogic.Billiard.Switch.equiv`, `ConservativeLogic.Billiard.Switch.encode_weightPreserving`, `ConservativeLogic.Billiard.Switch.card_validOutput`, and `ConservativeLogic.Billiard.Switch.no_raw_equiv`. Figures 17–18 remain open physical layouts: the caption says steering/timing mirrors and unit wires are not explicitly indicated, and no coordinates, common latency, bridge trace, trivial-crossover schedule, or clearance proof is available |
| §6 closing discussion | Every specially initialized billiard trajectory can be computed exactly and efficiently for arbitrary length | — | Out-of-model. There is no total global trajectory, continuous event semantics, numeric-exactness theorem, or complexity/cost model; the finite Figure 14 sampled trace does not establish this claim |
| §7.1 | Reversing gates and wires yields a semantic inverse for combinational networks | 7, 10 | Stage 7 proves exact feed-forward static inversion by `ConservativeLogic.Circuit.inverse_eval` and exact route reversal by `ConservativeLogic.Circuit.pathDelay_inverse_iff`. Stage 10 proves complete tick inversion, finite open-run retrodiction from terminal memory and complete outputs, and reversible closed finite iterates. These are semantic backward-determinism results, not a literal Figure 19 graph-reversal compiler, infinite-stream inverse without terminal state, oriented `t ↦ -t` execution, or physical time reversal |
| §7.1, Figs. 22–24 | Compute-copy-uncompute returns argument and scratch and emits `(y, not y)` | 8 | `ConservativeLogic.Ancilla.copyPair_spec` explicitly routes canonical `(a,0,1)` to physical `(a,1,0)`, `ConservativeLogic.Ancilla.copyRegister_spec` proves `(y,0ⁿ,1ⁿ) ↦ (y,y,¬y)`, and `ConservativeLogic.Ancilla.compute_copy_uncompute_spec` restores the complete packed scratch/source/argument state from any supplied `ConservativeLogic.Realization.Realizes` witness. The result register is exactly `2n` wires and no transient midpoint garbage remains |
| §7.1, Fig. 24 discussion | If conservation is waived, a reversible-only construction can start an `n`-bit result register at all zero and return only `y` | — | Open external-source variant. The paper cites Toffoli (1980) and leaves the primitives/construction unspecified; this library formalizes only the conservative `2n` register initialized as `(0ⁿ,1ⁿ)` and returning `(y,¬y)` |
| §7 introduction, §§7.1–7.2 | Garbage can be reduced to a returned copy of the argument, with claimed line-count and circuit-complexity consequences | 8, 9 | `ConservativeLogic.Ancilla.compute_copy_uncompute_spec` proves restoration and `ConservativeLogic.Ancilla.computeCopyUncompute_fredkinCount` proves count `2·count(circuit)+resultWidth`. The global workspace, line-count, and time asymptotics remain open (CL-012): Stage 9 supplies finite clean Fredkin-plus-structural-reindexing existence with an exactly returned, possibly mixed ancillary prefix but no global closed form or cost model |
| §7.1, Fig. 24(b) | The paper's restored `c` source/workspace can start all zero without loss of generality | 9 | Open (CL-011). The Margolus attribution has no precise proof or source. In the library layout, paper `c` is the consumed `source` later restored by compute-copy-uncompute; the separate already-returned `scratch` field is a stronger interface. No all-zero conversion theorem is claimed |
| §7.2, Fig. 25 | For any `f`, the initialized-slice map `(x,0ⁿ,1ⁿ) ↦ (x,f x,¬f x)` extends to a total conservative permutation | 5, 9 | Resolved semantically by the classical noncanonical Hamming-layer theorem `ConservativeLogic.exists_figure25_conservative`; `ConservativeLogic.figure25_fredkin_complete` then supplies a separate returned clean workspace and a paper-Fredkin-plus-structural-reindexing realization. The drawing itself still specifies only the initialized slice and no small canonical circuit |
| §7.2 | Worst-case synthesis of arbitrary `f` from a fixed bounded primitive basis may require scratch; claimed endpoint size/time tradeoffs | 9 | Open (CL-012) beyond finite existence and the local bounds `ConservativeLogic.Completeness.Adjacent.edgeClean_width_le`, `ConservativeLogic.Completeness.Adjacent.edgeWidth_le`, and exact `ConservativeLogic.Completeness.Adjacent.adjacentTranspositionCircuit_fredkinCount`. The asserted `exp(m)` sufficient and proportional-to-`m` least-usable endpoints omit family quantifiers, constants, output-width dependence, initialization/return conventions, and a formal cost model |
| §7.3, Fig. 26 | A direct same-register map `f : BitState m → BitState m` is semantically realizable by one arbitrary conservative primitive iff it is invertible and conservative | 9 | Resolved at exactly that monolithic same-width scope by `ConservativeLogic.direct_realization_iff`. It is kept separate from the fixed-basis theorem and does not cover unequal-width maps |
| §§2.5, 7.3 | Every invertible conservative finite function, and each fixed finite iterate, is Fredkin-realizable without visible garbage | 9, 10 | Stage 9 supplies corrected finite clean synthesis and refutes the no-ancilla reading. Stage 10's `ConservativeLogic.Sequential.ConservativeMachine.closedIterateEquiv`, `ConservativeLogic.Sequential.ConservativeMachine.closedIterate_reversible`, `ConservativeLogic.Sequential.ConservativeMachine.closedIterate_inverse_cancel`, and `ConservativeLogic.Sequential.ConservativeMachine.closedIterate_forward_cancel` package each supplied delayed-closure iterate as a reversible finite state map, while `ConservativeLogic.Sequential.ConservativeMachine.closedOrbit_weight` proves its pointwise weight invariant. No named theorem composes that sequential package with Fredkin synthesis; physical routing, all-zero ancillas, a global resource bound, and literal distributed-delay implementation remain absent |
| §7.3 | A fixed closed CPU function can synthesize arbitrary bounded input-output relationships by iteration | 10 | Open. `ConservativeLogic.Sequential.ConservativeMachine.closedIterateEquiv` only iterates a supplied transition; there is no universal CPU encoding, compiler, or simulation theorem showing that one fixed transition synthesizes arbitrary bounded behavior |
| §7.3 | Closed general-purpose computers have NAND-comparable gate complexity | 10 | Open. The paper supplies neither the scheduling theorem nor the gate/throughput/resource model required to compare complexity |
| Abstract and physical passages in §§1–2, 5–10 | Zero dissipation, entropy, energy, noise, topology, and physical-realizability conclusions | — | Out-of-model absent explicit physical state, dynamics, and thermodynamic models; these passages remain documentation only |

## Correction and Uncertainty Log

Each issue has a final Stage 12 status from the closed vocabulary `proved`,
`corrected`, `disproved`, `open`, and `out-of-model`.  An `open` item remains a
formal or source-research obligation; `out-of-model` is a final scope decision,
not an assertion that the paper's claim is false.
...
| CL-008 | open | Figure 18 says steering/timing mirrors and unit wires are not explicitly indicated, but identifies bridge crossovers and calls the others trivial; clearance and simultaneous collision scheduling are additional formalization obligations rather than quoted omissions. | Sampled routes certify mirror turns at distinct fixed sites, a same-position-endpoint detour whose boundary directions differ, and a simultaneous naked-crossing conflict. A one-tick stagger avoids equal centers but fails threshold-two sampled clearance; a two-tick stagger meets that sampled threshold. Figure 14 has a complete four-tick trace. Continuous clearance, a drop-in directed delay, the Figure 15 bridge, Figure 17, both Figure 18 layouts, and arbitrary layout composition remain unresolved until fixed coordinates/hardware, event-disjoint schedules, and all-input crossover/clearance proofs are supplied. |
| CL-009 | corrected | The spy/copy gadget needs one `0` and one `1` per copied result bit and emits both value and complement; Figure 22's drawn top-to-bottom `(0,1) → (a,not a)` data order conflicts with Table (2)'s zero-controlled swap. | `ConservativeLogic.Ancilla.copyPairInputWiring` equals `ConservativeLogic.PaperFredkin.dataSwap`; `ConservativeLogic.Ancilla.copyPair_physical_spec` checks `(a,1,0)`, and `ConservativeLogic.Ancilla.copyPair_spec` checks canonical `(a,0,1) ↦ (a,a,¬a)`. `ConservativeLogic.Ancilla.copyRegister_spec` lifts the corrected order to disjoint all-width spies, including width zero. |
| CL-010 | corrected | “Garbageless” still returns the original argument and uses/restores scratch. | From a supplied finite `Realizes` witness, `ConservativeLogic.Ancilla.compute_copy_uncompute_spec` returns the exact packed scratch, source, and argument, uncomputes the named transient garbage, and fills the separate ancillary register with `(target,bitwiseNot target)`. This does not mean absence of ancillary or argument-dependent output wires. |
| CL-011 | open | All-zero workspace sufficiency is attributed to Margolus without proof. The paper's Figure 24 scratchpad is the `c` source consumed into `g` and later restored, not the library's separate already-returned `scratch` block. | The corrected completeness theorem exposes an exact, possibly mixed clean initialization and does not imply an all-zero conversion. Any future conversion must cite or prove the construction and state its effects on width, count, and delay. |
| CL-012 | open | Scratchpad size claims use informal proportionality and an unstated cost model: `exp(m)` sufficiency at a least-time extreme and least usable Fredkin scratch proportional to `m`. | `ConservativeLogic.Completeness.Adjacent.edgeClean_width_le`, `edgeWidth_le`, and `adjacentTranspositionCircuit_fredkinCount` give local linear bounds and an exact count, but the final classical group-closure witness has no installed global closed form. A stronger theorem still needs circuit families, scratch/line and time measures, constants, output-width dependence, basis, initialization, and restoration. |
| CL-013 | corrected | “Any invertible conservative function” is Fredkin-realizable, but its ancilla and routing conventions are unclear; Figure 26 explicitly omits scratch for clarity, and §2.5 tacitly includes unit wires and identity. | `ConservativeLogic.fredkin_complete_conservative` records exact clean initialization/restoration, excludes `unitWire`, and permits explicit structural `WirePerm` routing. `ConservativeLogic.middleLayerSwap_not_circuit` disproves the no-ancilla reading, while a dependency-free exact group audit establishes width-four minimality through widths one to four. Structural reindexing is not called physical Fredkin synthesis. |
| CL-014 | corrected | Figure 25 specifies `F0` only on initialized inputs `(x,0ⁿ,1ⁿ)`; total invertibility and conservation do not follow “by definition,” and arbitrary-gate existence would not imply fixed-basis synthesis. | `ConservativeLogic.exists_conservative_extending_pair` and `ConservativeLogic.exists_figure25_conservative` perform a classical noncanonical extension inside each finite Hamming layer. `ConservativeLogic.figure25_fredkin_complete` separately applies corrected fixed-basis synthesis and exposes the additional returned clean workspace. |
| CL-015 | out-of-model | Claims about infinite blank tape/environment supplying constants and garbage space are not finite-circuit theorems. | No indefinitely extendible tape/environment or infinite reservoir is formalized in this finite library. Any future treatment requires a separately scoped infinite model. |
| CL-016 | out-of-model | Physical reversibility, entropy, and zero-dissipation conclusions do not follow from finite bijections alone. | These remain non-theorem commentary because the library defines no required physical state, dynamics, thermal environment, or thermodynamic quantities. |
| CL-017 | corrected | A serial/tensor/permutation syntax is not literally the paper's directed-graph circuit model, which includes feedback and open transducers with memory; structural wire renaming is also not automatically a physical wire/permutation circuit with delay. | Stages 4, 6, 7, and 9 use a corrected feed-forward grammar. Stage 10's `ConservativeLogic.Sequential.Network` accepts only a `Circuit.HasLatency 0` acyclic core and puts every stored wire in explicit state; feedback is register-separated. Figures 8 and 11 are manually reconstructed examples. No general literal-graph elaboration or physical routing theorem is claimed. |
| CL-018 | corrected | The paper calls Fredkin nonlinear without naming the algebraic structure. | `ConservativeLogic.BitState.xor`, `ConservativeLogic.BitState.falseState`, and `ConservativeLogic.XorLinear` define one explicit reconstruction; `ConservativeLogic.PaperFredkin.map_xor_counterexample` and `ConservativeLogic.PaperFredkin.map_not_xorLinear` prove failure. This is not presented as the paper's missing definition or as physical nonlinearity. |
| CL-019 | corrected | Figure 8 is asserted to realize a `J-K̄` flip-flop, but the paper gives no transition equation, initialization condition, or trace specification. | `ConservativeLogic.Sequential.Figure8.tick` and `ConservativeLogic.Sequential.Figure8.characteristic` prove the diagram-qualified reconstruction `(q,K̄,J) ↦ (if q then K̄ else J,q,if q then J else K̄)`. The initial bit is explicit, the first output is `Q`, the second is the paper's `?` garbage, and all eight rows plus hold/set/reset/toggle traces are audited. |
| CL-020 | corrected | Figures 20–23 assume a combinational forward network but do not specify delay balancing for the enlarged boundary after adding result-register constants and spy outputs. | `ConservativeLogic.Ancilla.compute_copy_uncompute_spec` is unconditional and static, while `ConservativeLogic.Ancilla.computeCopyUncompute_hasLatency_zero` requires a zero-latency supplied circuit. The non-public diagnostic `ConservativeLogic.Audit.Uncompute.unitWireUncompute_not_meetsPaperCombinationalTiming` exhibits delay-two and delay-zero paths through the actual unpadded builder. No positive-latency or padding theorem is claimed. |
| CL-021 | corrected | Section 2.5's `N₁` trajectory-invariant wording is sound for a closed circuit's complete stored state, but an open transducer can exchange true bits with its environment. | `ConservativeLogic.Sequential.ConservativeMachine.tick_weight_balance`, `ConservativeLogic.Sequential.ConservativeMachine.run_prefix_weight_balance`, and `ConservativeLogic.Sequential.ConservativeMachine.closedOrbit_weight` prove complete-boundary flux and the closed invariant. The delay-cell audit concretely changes memory weight while preserving complete boundary flux. Only the delayed closed `memory ++ loopRegister` state has the all-time invariant. |
| CL-022 | corrected | Integral grid occupancy is not a deterministic billiard state: velocity/directed channel and fixed hardware are required, and the paper supplies no total rule for shared-ball, multi-contact, mirror-collision, or target conflicts. | Stage 11 installs only a selected four-channel scattering rule on `ConservativeLogic.Billiard.Collision.AllowedState` and finite products of independent owned sites. Public `ConservativeLogic.Billiard.ScatteringLayer.illegalThreeBallMask_not_allowed` and diagnostic `ConservativeLogic.Audit.Billiard.tripleCandidates_conflict` reject two concrete missing-event cases. Directed sampled routes are a stated correction; no theorem claims a global continuous hard-ball update or legal-orbit closure under arbitrary routing. |
| CL-023 | corrected | The switch branch pair `(c x, ¬c x)` is one-hot-or-empty, not a general dual-rail pair `(b,¬b)`; mirrors also redirect balls but do not supply occupied Boolean constants. | Stage 11 keeps all three switch rails and the exact valid-output subtype. The only NOT slice is `ConservativeLogic.Billiard.Interaction.not_with_true_source`, whose documentation names the occupied `q=true` source. No dual-rail attribution, free constant, reservoir, or repeated source-stream theorem is inferred from a mirror. |
| CL-024 | open | Section 7.3 attributes the fixed-basis result to B. Silver, while the acknowledgments name D. Silver; the paper gives no proof or bibliography entry resolving the attribution. | The mathematical claim has the corrected CL-013 theorem, but the historical B./D. attribution remains unresolved from the available paper text and repository sources. |
...
```text
ConservativeLogic/
  Audit/Guardrails.lean     -- private representation and dependency probe
  State/Core.lean           -- states, weights, and stable state-width transport
  Reversible/Core.lean      -- finite reversible and conservative maps
  Reversible/Independence.lean -- semantic independence witnesses
  Audit/Finite.lean         -- non-public finite-state boundaries and axiom audit
  ...
  Audit/Fredkin.lean        -- non-public truth rows, boundaries, and axiom audit
  Circuit/Syntax.lean       -- arity-safe syntax, width transport, and wire chains
  Circuit/Semantics.lean    -- evaluation and preservation theorems
  Circuit/Structural.lean   -- transport semantics and block reindexing
  ...
  Simulation/Fredkin.lean   -- compiler plus compatibility structural forwarders
...
`Simulation.castCircuit`, `Simulation.eval_castCircuit`,
`Simulation.middleSwapWiring`, and
...
- `zeroCount_zero`: the derived false-wire count handles the width-zero
  boundary explicitly.
...
- `Circuit.cast` and `Circuit.eval_cast`: dependent width transport changes
  neither circuit syntax nor evaluation.
- `Circuit.middleSwapWiring` and `Circuit.middleSwapWiring_on_append`: the
  compiler's canonical four-block structural route has a proved active action.
...
- `Ancilla.zeroRegister`, `Ancilla.oneRegister`, `Ancilla.bitwiseNot`,
  `Ancilla.resultRegisterInput`, and `Ancilla.resultRegisterOutput`: the
  circuit-independent result-register states used by uncompute and semantic
  completion, together with their input/output and complement weight laws.
...
- `Circuit.FredkinStructural`, `CleanFredkinRealization`, and
  ...
  zero path latency.
- `ConservativeLogic.CleanFredkinRealization.wireConjugate`: generic clean structural conjugation
  owned with the clean witness type rather than the adjacent-synthesis macro.
...
- `ConservativeLogic.Completeness.Adjacent.patternMatch_spec`,
  `ConservativeLogic.Completeness.Adjacent.edgeClean_width_le`,
  `ConservativeLogic.Completeness.Adjacent.edgeWidth_le`,
  `ConservativeLogic.Completeness.Adjacent.adjacentTranspositionCircuit_spec`,
  `ConservativeLogic.Completeness.Adjacent.adjacentTranspositionCircuit_fredkinCount`, and
  `ConservativeLogic.Completeness.Adjacent.adjacentTranspositionClean`: exact local Johnson-edge
  semantics,
...
- `Conservative.weightLayer_exchange_connected`,
  ...
  `ConservativeLogic.Completeness.Adjacent.singleExchangeClean`: layer connectivity and clean
  routed state exchanges.

...
| CL-008 | Figure 18 explicitly omits steering/timing mirrors and unit wires, but identifies bridge crossovers and calls the others trivial; clearance and simultaneous collision scheduling are additional formalization obligations rather than quoted omissions. | Partially resolved and deliberately open after Stage 11. Sampled routes now certify mirror turns, a finite detour, a simultaneous naked-crossing conflict, and one staggered clear use; Figure 14 has a complete four-tick trace. Continuous clearance, the Figure 15 bridge, Figure 17, both Figure 18 layouts, and arbitrary layout composition remain unresolved until fixed coordinates/hardware, event-disjoint schedules, and all-input crossover/clearance proofs are supplied. |
...
- `Grid.fourTickDetour_reflects`, `simultaneous_crossing_conflict`, and
  `staggered_crossing_clear`: explicit sampled mirror/timing obligations.

...
| CL-009 | corrected | The spy/copy gadget needs one `0` and one `1` per copied result bit and emits both value and complement; Figure 22's drawn top-to-bottom `(0,1) → (a,not a)` data order conflicts with Table (2)'s zero-controlled swap. | `ConservativeLogic.Ancilla.copyPairInputWiring = ConservativeLogic.PaperFredkin.dataSwap`; `ConservativeLogic.Ancilla.copyPair_physical_spec` checks `(a,1,0)`, and `ConservativeLogic.Ancilla.copyPair_spec` checks canonical `(a,0,1) ↦ (a,a,¬a)`. `ConservativeLogic.Ancilla.copyRegister_spec` lifts the corrected order to disjoint all-width spies, including width zero. |

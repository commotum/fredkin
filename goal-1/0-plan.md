...
| CL-008 | Figure 18 says steering/timing mirrors and unit wires are not explicitly indicated, but identifies bridge crossovers and calls the others trivial; clearance and simultaneous collision scheduling are additional formalization obligations rather than quoted omissions. | Partially resolved and deliberately open after Stage 11. Sampled routes now certify mirror turns at distinct fixed sites, a finite detour, and a simultaneous naked-crossing conflict. A one-tick stagger avoids equal centers but fails threshold-two sampled clearance; a two-tick stagger meets that sampled threshold. Figure 14 has a complete four-tick trace. Continuous clearance, the Figure 15 bridge, Figure 17, both Figure 18 layouts, and arbitrary layout composition remain unresolved until fixed coordinates/hardware, event-disjoint schedules, and all-input crossover/clearance proofs are supplied. |

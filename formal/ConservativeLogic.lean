...
The current API provides finite Boolean states, Hamming weight and its derived
fixed-width false-wire count, reversible and
...
circuit grammar, static conservative evaluation, arbitrary finite one-bit wire
chains with exact latency, and path-latency predicates.

...
selected finite ancillary width, exact possibly mixed initialization and
restoration, a no-unit-wire syntax certificate, and zero latency.  A width-four odd
weight-layer transposition proves that the false same-width/no-ancilla reading
is not generated by this circuit grammar.  No all-zero-scratch conversion,
